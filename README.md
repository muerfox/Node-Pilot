...
The backend suite (185 tests) runs against SQLite with a faked Redis (no
...
and NIC bandwidth limits actually reaching the domain XML, all with the
agent
RPC layer mocked. The agent suite (78 tests, +3 skipped when `qemu-img`
...
format selection, per-VLAN bridge resolution, and NIC bandwidth
elements), the storage backends'
...
a 147 KB gzipped bundle (noVNC accounts for most of that). 271
