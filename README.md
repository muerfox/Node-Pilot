...
The backend suite (71 tests) runs against SQLite with a faked Redis (no
...
metrics ingest, webhook delivery's SSRF-redirect protection, disk-format
propagation onto the domain XML, backup create/restore payload
correctness, and backup-target credential masking, all with the agent
...
that). 127 automated tests pass as of this build.
